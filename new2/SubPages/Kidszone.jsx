...
const Kidszone = () => {
  ...
  const renderPosters = (category) => {
    ...
    return videos.map((video, index) => {
        ...
        const videoPath = `${serverIp}/kidszone/${category}/${videoName}`;
        const posterPath = `${serverIp}/kidszone/${category}/${videoName.replace('.mp4', '.jpg')}`;
  ...
  const handleVideoClick = (videoName, index) => {
    ...
    if (videoRef.current) {
      ...
      videoRef.current.src = `${serverIp}/kidszone/${selectedCategory}/${videoName}`;
    }
  };
  ...
  useEffect(() => {
    if (currentVideo && videoRef.current) {
      const videoUrl = `${serverIp}/kidszone/${selectedCategory}/${currentVideo}`;
