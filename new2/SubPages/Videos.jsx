...
const Videos = () => {
  ...
  const renderPosters = (category) => {
    ...
    return videos.map((video, index) => {
        ...
        const videoPath = `${serverIp}/videos/${category}/${videoName}`;
        const posterPath = `${serverIp}/videos/${category}/${videoName.replace('.mp4', '.jpg')}`;
  ...
  const handleVideoClick = (videoName, index) => {
    ...
    if (videoRef.current) {
      ...
      videoRef.current.src = `${serverIp}/videos/${selectedCategory}/${videoName}`;
    }
  };
